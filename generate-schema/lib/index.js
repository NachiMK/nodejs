...
function findUpdatePopularformat(stat) {
  ...
  if (_.size(popularFormats) > 0) {
    ...
    if (_.size(popularFormats) > 1) {
      ...
      _.forEach(popularFormats, format => {
        ...
        } else if (popularFormat === "date" && format.FormatName === "date-time") {
          popularFormat = format.FormatName;
        } else if (popularFormat !== "string" && format.FormatName !== "string") {
  ...
  stat.schemaUpdateFunctions.push(s => s.format = popularFormat || stat.format);
}
