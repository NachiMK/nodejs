CREATE OR REPLACE FUNCTION public.validate_json_schema(schema jsonb, data jsonb, root_schema jsonb DEFAULT NULL) RETURNS boolean AS $f$
DECLARE
  prop text;
  item jsonb;
  path text[];
...
GRANT ALL on FUNCTION public.validate_json_schema(jsonb, jsonb, jsonb) TO odsddb_role;
GRANT ALL on FUNCTION public.validate_json_schema(jsonb, jsonb, jsonb) TO public;