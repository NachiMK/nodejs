...
if [ "${create}" = "CreateDB:TRUE" ]; then 
    echo "Creating Database:applog_${stagename}"
    echo "Create:"$create
    psql ${dbhostname} -d postgres -c "DROP DATABASE IF EXISTS applog_${stagename};"
    psql ${dbhostname} -d postgres -c "CREATE DATABASE applog_${stagename} WITH TEMPLATE = template0 ENCODING = 'UTF8' LC_COLLATE = 'en_US.UTF-8' LC_CTYPE = 'en_US.UTF-8';"
else
...
if [ "${resetdb}" = "ResetDB:TRUE" ]; then 
    for filename in Tables/*.sql; do
        ...
        psql ${dbhostname} -d applog_${stagename} -f "$filename"
    done
else
