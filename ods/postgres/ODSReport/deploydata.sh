...
app_role="odsreport_app_role_${stagename}"
app_user="odsreport_app_user_${stagename}"
db="odsreport_${stagename}"
...
function grantPermissionToSchema {
    ...
    GRANT USAGE ON SCHEMA '$1' to '${app_role}';
    ALTER DEFAULT PRIVILEGES FOR USER '${2}' IN SCHEMA '$1' GRANT SELECT ON TABLES TO '${2}';
...
function createdbLink {
    deployCmd "CREATE SERVER ods_${1}_server FOREIGN DATA WRAPPER postgres_fdw
            OPTIONS (host 'localhost', port '5432', dbname 'odsdynamodb_${1}');"
    deployCmd "CREATE USER MAPPING FOR hixme_ods_root
            SERVER ods_${1}_server
            OPTIONS (user 'hixme_ods_root', password '${2}');"
    deployCmd "CREATE USER MAPPING FOR odsreport_app_role_int
            SERVER ods_${1}_server
            OPTIONS (user 'hixme_ods_root', password '${2}');"
    deployCmd "SELECT dblink_connect('ods_${1}_server');"
    deployCmd "GRANT USAGE ON FOREIGN SERVER ods_${1}_server TO odsreport_app_role_int;"
}
...
if [ "${create}" = "CreateDB:TRUE" ]; then 
   ...
   for filename in Tables/*.sql; do
    ...
    # create link
    createdbLink "${stagename}" "${PGPASSWORD}"
else
