...
const getSchemaDefaultValue = (type) => {
  ...
  return type && Object.keys(typeDefaultValue).includes(type) ? defaultValue : ''
}
...
export const parseSchemaJSON = (data, parentKey) => {
  ...
  if (data && data instanceof Object) {
    ...
    const setType = type && !restData.$schema ? getSchemaType(type) : null

    const result =
      parentKey !== 'items' && setType && setType !== 'object' && setType !== 'array'
        ? Object.assign(
            {
              type: setType,
...
export default async ({
  ...
  try {
    if (!Datafile || Datafile.length === 0 || Datafile.substring(0, 2) !== 's3') {
      throw new Error(
        `Datafile file path invalid with value ${Datafile}. Please provide a valid AWS S3 path.`
      )
    }
    if (!Output || Output.length === 0 || Output.substring(0, 2) !== 's3') {
      throw new Error(
        `Output file path invalid with value ${Output}. Please provide a valid AWS S3 path.`
      )
    }
    if (
      !S3RAWJsonSchemaFile ||
      S3RAWJsonSchemaFile.length === 0 ||
      S3RAWJsonSchemaFile.substring(0, 2) !== 's3'
    ) {
      throw new Error(
        `S3RAWJsonSchemaFile file path invalid with value ${S3RAWJsonSchemaFile}. Please provide a valid AWS S3 path.`
      )
    }
...
async function getCombinedSchema(schemaFromData, S3RAWJsonSchemaFile) {
  ...
  if (schemaFromData && schemaFromData.schema && S3RAWJsonSchemaFile) {
    ...
    throw new Error(
      `Cannot combine schema either schemaFromData : ${schemaFromData} is null or S3RAWJsonSchemaFile Path is null: ${S3RAWJsonSchemaFile}`
    )
  }
  return resp
}
