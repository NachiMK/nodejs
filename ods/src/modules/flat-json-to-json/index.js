...
function convertSimpleArrayToObjects(arrayOfSimpleDataTypes, parentId, parentLevel, parentName, myName, parentObject, debug = 0) {
  let retArray = [];
  // is an array check
...
function addParentIdToChildObject(parentId, parentLevel, parentObject, parentName, childObj, debug = 0) {
  if (parentLevel < 0) {
    parentLevel = 0;
  }
  ...
  if ((childObj) && (!_.isUndefined(parentId))) {
    ...
    if ((parentObject) && (parentLevel > 0)) {
      ...
      childObj[globalDefaultParentPath] = parentObject[globalDefaultParentPath] + childObj[globalDefaultParentPath];
      childObj[gloablDefaultParentUri] = parentObject[gloablDefaultParentUri] + childObj[gloablDefaultParentUri];
    }
...
function updateUri(objToUpdate, debug = 0) {
  if (!_.isUndefined(objToUpdate) && _.isObject(objToUpdate)) {
    if (objToUpdate[globalDefaultUri]) {
      objToUpdate[globalDefaultUri] = objToUpdate[gloablDefaultParentUri] + objToUpdate[globalDefaultUri];
    }
    if (objToUpdate[globalDefaultUriPath]) {
      objToUpdate[globalDefaultUriPath] = objToUpdate[globalDefaultParentPath] + objToUpdate[globalDefaultUriPath];
    }
  }
