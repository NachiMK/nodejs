...
export async function DynamoStreamEventsToS3(StreamEventsToS3Param = {}) {
  ...
  const saveToS3Status = await SaveDynamoRowsToS3(
    changedRecords,
    TableKeyName,
    S3BucketName,
    AppendDateTime,
...
export async function SaveDynamoRowsToS3(
  Rows,
  KeyName,
  S3BucketName,
  AppendDateTime = true,
...
async function ValidateParams(StreamEventsToS3Param = {}) {
  const errorMessage = `Invalid Values Passed in for StreamEventsToS3Param = {${JSON.stringify(
    StreamEventsToS3Param,
    null,
    2
  )}}`
...
async function getDataTimeStamp(timeformat = 'YYYYMMDD_HHmmssSSS') {
  timeformat =
    _.isEmpty(timeformat) || _.isUndefined(timeformat) ? 'YYYYMMDD_HHmmssSSS' : timeformat
  return moment().format(timeformat)
}

export async function getJSONFileKeyName(
  FilePrefix,
  AppendDtTm = true,
  DateTimeFormat = 'YYYYMMDD_HHmmssSSS'
) {
  FilePrefix =
    _.isUndefined(FilePrefix) || _.isEmpty(FilePrefix)
      ? 'UNKNOWN_'
      : FilePrefix.replace('.json', '')
