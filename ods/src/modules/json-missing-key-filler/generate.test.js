...
describe('Json Missing Key Filler - Unit Tests', () => {
  ...
  it.only('Json Missing Key Filler should create a file in s3', async () => {
    ...
    event.S3OutputKey = `unit-test/clients/test-clients-UniformJson-${moment().format('YYYYMMDD_HHmmssSSS')}.json`;
    const objModule = new JsonDataNormalizer(event);
    await objModule.getUniformJsonData();
    console.log('S3UniformJsonFile File Path', JSON.stringify(objModule.Output.S3UniformJsonFile, null, 2));
