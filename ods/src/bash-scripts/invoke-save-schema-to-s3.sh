...
lambdafile="SchematoS3_InvokeArgs_${DATE}.json"
...
echo ${payload} >> ${lambdafile}
echo "Invoke Args file: ${lambdafile}"
echo $(aws lambda invoke-async --function-name ods-service-${stage}-dba-save-schema-to-s3 --invoke-args "${lambdafile}")
