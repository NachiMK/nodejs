...
function extractStatusAndAttributes(moduleResponse, task, taskResponse) {
  if (IsStatusSuccess(moduleResponse)) {
    ...
    if (moduleResponse.files) {
      ...
      taskResponse.error = new Error('Json to CSV returned Success, but S3CSVFiles path is missing.');
    }
  } else {
    // throw an error
    taskResponse.Status = TaskStatusEnum.Error.name;
    taskResponse.error = new Error(`Saving Json to CSV Failed. Retry Process. Error: ${moduleResponse.error}`);
  }
}
...
function getInput(task) {
  ...
  const input = {
    Datafile: task.getTaskAttributeValue('S3CSVBucketName').replace('https://s3-us-west-2.amazonaws.com/', 's3://'),
    FilePrefix: myFilePrefix,
    Output: `s3://${task.getTaskAttributeValue('S3CSVBucketName')}/${s3FileKeyPrefix.replace(myFilePrefix, '')}`,
    Overwrite: 'yes',
  };
