...
export const getTablesFromDB = async (params, TableNameFilter = '') => {
  ...
  TableList = TableList.concat(
    items.TableNames.filter((name) =>
      name.toLocaleLowerCase().includes(TableNameFilter.toLocaleLowerCase())
    )
  )
...
export const getTableInfoAllTables = async (stagename) => {
  ...
  tablelist = await Promise.all(
    tbls.map(async (tablename) => {
...
export const HasHistoryTable = async (tablename) => {
  ...
  if (descTableData) {
    ...
    try {
      ...
      console.log(
        `Error getting V1histTable meta data, Table: ${v1HistTableName}, Error:${JSON.stringify(
          err,
          null,
          2
        )}`
    ...
    try {
      ...
      console.log(
        `Error getting v2HistTable meta data, Table: ${v2HistTableName}, Error:${JSON.stringify(
          err2,
          null,
          2
        )}`
...
export const EnableHistoryOnTables = async (EnableTableList) => {
  if (EnableTableList) {
    const objRetArray = await Promise.all(
      EnableTableList.map(async (tablename) => {
...
export const createHistoryTable = async (tablename) => {
  ...
  if (tblstatus) {
    ...
    console.log(
      `History Table ${historyTable} Exists Check Desc:${JSON.stringify(histTableExists, null, 2)}`
    )
    if (histTableExists) {
      console.log(
        `History Tables Exists:${historyTable}, Status: ${histTableExists.Table.TableStatus}`
      )
