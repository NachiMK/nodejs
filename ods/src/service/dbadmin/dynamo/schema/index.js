...
import {
  GetDynamoTablesToRefreshSchema as dataGetTableToRefresh,
  UpdateSchemaFile as dataUpdateSchemaFile,
} from '../../../../data/ODSConfig/DynamoTableSchema'
...
const CreateSchema = async (tableRequest) => {
  ...
  const { Bucket, Key } = s3FileParser(S3JsonSchemaPath)
  const FileKey = Key
    ? Key.replace(pattern, `-${moment().format('YYYYMMDD_HHmmssSSS')}.json`)
    : `dynamotableschema/${DynamoTableName.replace(/[proddevint]+-+/i, '')}-${moment().format(
        'YYYYMMDD_HHmmssSSS'
      )}.json`
  ...
  try {
    if (tableRequest) {
      ...
      if (schemaResp && schemaResp.Status && schemaResp.Status === 'success') {
        ...
        if (s3Resp && s3Resp.Status && s3Resp.Status === 'success' && s3Resp.S3FilePath) {
          const dbResp = await dataUpdateSchemaFile(
    ...
    ODSLogger.log(
      'error',
      `Error in CreateSchema for table ${DynamoTableName}, Error: ${err.message}`
    )
    throw err
  }
  return s3Resp
