...
import {
  CreatingDataPipeLineTaskError,
  GettingPendingTaskError,
  GetTaskAttributeError,
} from '../../../modules/ODSErrors/DataPipeLineTaskQueueError'
...
async function createDataPipeLineTaskProcessHistory(TableName, S3DataPipeLineTaskQueueId) {
  ...
  try {
    ...
    if (retRS.rows.length > 0) {
      ...
      throw new Error(
        'Error creating DataPipeLineTaskQueue for Processing History, DB Call returned 0 rows.'
      )
    }
    ODSLogger.log('info', 'create DataPipeLinetaskQueue status: %j', retRS)
  } catch (err) {
...
export async function GetPendingPipeLineTask(TableName) {
  ...
  try {
    ...
    const retRS = await executeQueryRS(params)
    if (retRS.rows.length > 0) {
      pendingTasks = await Promise.all(
      ...
      ODSLogger.log(
        'warn',
        `There are NO Pending DataPipeLineTask for Table:${TableName}, DB Call returned 0 rows.`
      )
    }
    ODSLogger.log(
      'info',
      `No Of Pendings Tasks ${
        pendingTasks && pendingTasks.length ? pendingTasks.length : 0
      } for Table: ${TableName}`
    )
    ODSLogger.log(
      'debug',
      `Pendings Tasks ${JSON.stringify(pendingTasks, null, 2)} for Table: ${TableName}`
    )
  } catch (err) {
    pendingTasks = []
...
export async function GetPipeLineTaskQueueAttribute(taskId) {
  ...
  try {
    ...
    const retRS = await executeQueryRS(params)
    if (retRS.rows.length > 0) {
      taskAttributes = await Promise.all(
        retRS.rows.map(async (dataRow) => {
          const retVal = {
      ...
      ODSLogger.log(
        'warn',
        `There are NO Attributes for DataPipeLineTask :${taskId}, DB Call returned 0 rows.`
      )
    }
    ODSLogger.log('info', `No Of Task Attributs ${taskAttributes.length} for Task Id: ${taskId}`)
    ODSLogger.log(
      'debug',
      `Task Attributes ${JSON.stringify(taskAttributes, null, 2)} for Task Id: ${taskId}`
    )
  } catch (err) {
    taskAttributes = []
...
export async function GetPipeLineTaskStatus(taskId) {
  ...
  try {
    ...
    ODSLogger.log(
      'debug',
      `Status Response ${JSON.stringify(retRS, null, 2)} for Task Id: ${taskId}`
    )
