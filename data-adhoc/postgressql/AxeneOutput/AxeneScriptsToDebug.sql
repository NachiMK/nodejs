SELECT * FROM "AxeneBatch" WHERE "ID" >= 114;

SELECT "AxeneBatchID", "FileName", "Status", "StartDate", "EndDate" 
FROM "AxeneBatchFiles" WHERE "AxeneBatchID" = 114 --IN (SELECT "ID" FROM "AxeneBatch" WHERE "ID" >= 113)
AND "Status" != 'started'
ORDER BY "FileName"

SELECT "AxeneBatchID", "FileName", "Status", "StartDate", "EndDate" 
FROM "AxeneBatchFiles" WHERE "AxeneBatchID" = 115 --IN (SELECT "ID" FROM "AxeneBatch" WHERE "ID" >= 113)
AND "Status" != 'started'


SELECT * FROM "AxeneOutputValues" WHERE "BatchID" IN (SELECT CAST("ID" AS VARCHAR) FROM "AxeneBatch" WHERE "ID" >= 115);
SELECT * FROM "AxenePlanBenefitErrors" WHERE "AxeneBatchFileID" = '115';
SELECT "PlanID", "FileName", Count(*) FROM "AxeneOutputValues" WHERE "BatchID" IN (SELECT CAST("ID" AS VARCHAR) FROM "AxeneBatch" WHERE "ID" >= 115)
GROUP BY "PlanID", "FileName"
HAVING COUNT(*) = 2;
-- 108_8e8e412a-3816-4534-b9ed-d6a1e7d85d42_2500_p

SELECT 'cp ' || "FileName" || '.json /Users/Nachi/Documents/work/git/axene-scripts/axene-error-converter/inputs/' FROM "AxeneBatchFiles" AS AO WHERE "AxeneBatchID" = 115
...
SELECT "FileName" FROM "AxeneOutputValues" WHERE "BatchID" IN (SELECT CAST("ID" AS VARCHAR) FROM "AxeneBatch" WHERE "ID" >= 115)
GROUP BY "PlanID", "FileName"
HAVING COUNT(*) > 1)
ORDER BY "FileName";

SELECT * FROM "AxeneOutputValues" WHERE "ID" IN (
SELECT MIN("ID") FROM "AxeneOutputValues" WHERE "BatchID" IN (SELECT CAST("ID" AS VARCHAR) FROM "AxeneBatch" WHERE "ID" >= 115)
GROUP BY "PlanID", "FileName"
HAVING COUNT(*) > 1)
ORDER BY "FileName";


SELECT "AxeneBatchID", Count(*) AS Cnt FROM "AxeneBatchFiles" WHERE "AxeneBatchID" IN (SELECT "ID" FROM "AxeneBatch" WHERE "ID" >= 115) GROUP  BY "AxeneBatchID";
SELECT "BatchID" as "AxeneBatchID", Count(*) AS Cnt FROM "AxeneOutputValues" WHERE "BatchID" IN (SELECT CAST("ID" AS VARCHAR) FROM "AxeneBatch" WHERE "ID" >= 115) GROUP  BY "BatchID";
SELECT "AxeneBatchFileID", Count(*) AS Cnt FROM "AxenePlanBenefitErrors" WHERE "AxeneBatchFileID" IN (SELECT "ID" FROM "AxeneBatch" WHERE "ID" >= 115) GROUP  BY "AxeneBatchFileID";
...
SELECT * FROM "AxeneBatchFiles" WHERE "AxeneBatchID" IN (SELECT "ID" FROM "AxeneBatch" WHERE "ID" >= 115)
AND "FileName" LIKE '111_47db707a-1c30-4588-828a-ff27d35a1879_2000_p%';

SELECT * FROM "AxeneOutputValues" WHERE "BatchID" like '115'
...
SELECT 238584 + Rbr, "AxeneBatchID", "FileName", "Status", "StartDate", "EndDate"  FROM cte
...
INSERT INTO "AxeneBatch" ("Event")
VALUES ('{"body": {}, "query": {"Year": "2018", "State": "AK"}, "stage": "${process.env.STAGE}", "queryStringParameters": {"Year": "2018", "State": "CA"}}')
...
SELECT * FROM "AxeneBatchFilesHistory" WHERE "FileName" = '112_44299941-a930-4d43-af30-0f4503489364_0_s';
