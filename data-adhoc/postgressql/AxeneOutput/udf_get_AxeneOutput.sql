...
CREATE OR REPLACE FUNCTION udf_get_AxeneOutput(batchid VARCHAR(255)) RETURNS SETOF vw_AxeneOutputFormat AS $$
    ...
    (
        SELECT   AO."ID"
                ...
                ,AO."PlanID"
                ,AO."ActuarialValue"
                ,'HixmeValuePlus' || REPLACE(REPLACE(REPLACE(REPLACE("FileName", "PlanID", ''), "BatchID", ''), "ModeledMetalTier", ''), '_', '') as "HixmePlusColName"
                ,CAST(REPLACE(REPLACE(REPLACE(REPLACE("FileName", "PlanID", ''), "BatchID", ''), "ModeledMetalTier", ''), '_', '') AS NUMERIC) as "HixmePlusAddOnValue"
                ,AO."HixmeValue"
